...
void RedAeroportuaria::Vuelo::verAerolineas(std::list<std::string> &aerolineas) const
// O(k), siendo k el número de aerolíneas.
{
...
// Complejidad espacial: O(max{n, k}), k debido a que almaceno un mapa con las aerolíneas y sus asientos totales al realizar una
// copia del costo del arco y n, a que en "existeArco" utilizo una lista con los aeropuertos adyacentes al de origen que, en el
// peor de los casos, tendrá tantos elementos como número de aeropuertos.
...
// Complejidad temporal: O(max{k*n!*n*log n, (k^2)*n!*n*log k), siendo n el número de aeropuertos y k, el número de aerolíneas.
...
// Complejidad temporal: O(max{n!*n*log n, n!*n*k log k}), siendo n el número de aeropuertos y k, el número de aerolíneas. Esto
// debido a que se realiza una búsqueda exhaustiva y, en cada recorrido, invocará a "existeReserva" y a "asientosReservados", cuya
// complejidad es la indicada.
// Complejidad espacial: O(max{n, k}), ya que utiliza dos estructuras lineales: vuelo (su mapa interno), que en el
...
// elementos como aeropuertos haya.
{
    ...
    else
    {
        ...
        for (const auto &arco : arcos)                                                       // O(n)
        {
            ...
            if (existeReserva(origen, arco.devolverAdyacente(), aerolinea))                                   // O(max{log n, log k})
            {
                asientosReservados = devolverAsientosReservados(origen, arco.devolverAdyacente(), aerolinea); // O(max{log n, log k})
            }
            else
                asientosReservados = 0;
...
// Complejidad temporal: O(max{n! * k log k, n! * n}), siendo n el número de aeropuertos en la red aeroportuaria y k, el número de aerolíneas.
// Esto debido a que invoca a "calcularCircuitoMinimo".
...
// tendrá tantos elementos como aeropuertos - 1.
{
    ...
    else
    {
        ...
        for (const auto &arco : adyacentesVerticeInicio)                                // O(m). En el peor de los casos, m = n.
        {
